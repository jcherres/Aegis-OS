...
# =============================================================================
# HTML — LOGIN DEL PANEL DE ADMINISTRACIÓN
# =============================================================================
HTML_ADMIN_LOGIN = """
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<title>Aegis · Acceso administrador</title>
<script src="https://cdn.tailwindcss.com"></script>
<link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;800&display=swap" rel="stylesheet">
<style>
  body { background:#020617; font-family:'JetBrains Mono',monospace; }
  .glass-card { background:rgba(15,23,42,0.55); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border:1px solid rgba(30,41,59,0.8); }
</style>
</head>
<body class="min-h-screen flex items-center justify-center">
  <form method="post" action="/admin/login" class="glass-card rounded-2xl p-10 w-full max-w-sm space-y-5">
    <h1 class="text-cyan-400 text-lg tracking-widest">AEGIS · ACCESO PRIVADO</h1>
    <input name="usuario" placeholder="usuario" class="w-full bg-slate-900/70 border border-slate-800/80 rounded-lg px-4 py-2 text-slate-200 outline-none focus:border-cyan-500" autocomplete="username">
    <input name="password" type="password" placeholder="contraseña" class="w-full bg-slate-900/70 border border-slate-800/80 rounded-lg px-4 py-2 text-slate-200 outline-none focus:border-cyan-500" autocomplete="current-password">
    <button class="w-full bg-cyan-600 hover:bg-cyan-500 transition rounded-lg py-2 text-slate-950 font-bold">ENTRAR</button>
  </form>
</body>
</html>
"""
...
# =============================================================================
# HTML — PANEL MAESTRO PRIVADO (sólo accesible tras login autenticado)
# =============================================================================
HTML_MASTER_PANEL = """
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<title>Aegis · Panel Maestro</title>
<script src="https://cdn.tailwindcss.com"></script>
...
<body class="min-h-screen p-8 space-y-8">
  <header class="flex items-center gap-3">
    <i data-lucide="shield-check" class="text-cyan-400"></i>
    <h1 class="text-xl tracking-widest text-cyan-400">PANEL MAESTRO · SERVIDOR</h1>
  </header>

  <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="glass-card rounded-2xl p-6 col-span-2">
      <h2 class="text-sm text-slate-400 mb-3">RAM del servidor (histórico en vivo)</h2>
      <canvas id="ramChart" height="120"></canvas>
    </div>
    <div class="glass-card rounded-2xl p-6 space-y-3">
      <h2 class="text-sm text-slate-400">Estado de la casa</h2>
      <p>Días restantes: <span id="diasRestantes" class="text-cyan-400 font-bold">--</span></p>
      <p>Integrantes registrados: <span id="totalIntegrantes" class="text-cyan-400 font-bold">--</span></p>
    </div>
  </section>

  <section class="glass-card rounded-2xl p-6 max-w-md space-y-4">
    <h2 class="text-sm text-slate-400">Acreditar días de licencia</h2>
    <p class="text-xs text-slate-500">Usa esto tras confirmar manualmente una transferencia o Yape.</p>
    <div class="flex gap-3">
      <input id="diasInput" type="number" min="1" value="30" class="flex-1 bg-slate-900/70 border border-slate-800/80 rounded-lg px-4 py-2 outline-none focus:border-cyan-500">
      <button onclick="acreditar()" class="bg-cyan-600 hover:bg-cyan-500 transition rounded-lg px-5 font-bold text-slate-950">Acreditar</button>
    </div>
    <p id="msgAcreditar" class="text-xs text-emerald-400"></p>
  </section>
  ...
  <script>
    ...
    const ramChart = new Chart(ctx, {
      type: 'line',
      data: { labels: [], datasets: [{ label: 'RAM %', data: [], borderColor: '#22d3ee', tension: 0.3 }] },
    ...
    async function actualizarEstado() {
      ...
      document.getElementById('diasRestantes').textContent = data.casa.dias_restantes;
      document.getElementById('totalIntegrantes').textContent = data.total_integrantes;

      const ahora = new Date().toLocaleTimeString();
      ramChart.data.labels.push(ahora);
      ramChart.data.datasets[0].data.push(data.ram_servidor);
    ...
    async function acreditar() {
      ...
      const res = await fetch('/api/admin/acreditar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dias })
      });
      const data = await res.json();
      document.getElementById('msgAcreditar').textContent = res.ok
        ? `Listo. Nuevo saldo: ${data.dias_restantes} días.`
        : 'Error al acreditar.';
      actualizarEstado();
    }
