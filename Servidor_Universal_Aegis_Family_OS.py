...
=====================================
AEGIS FAMILY OS · Servidor Universal Políglota v12.0
---------------------------------------------------
Un solo archivo = Backend FastAPI + SQLite/PostgreSQL + Dashboard HTML/CSS/JS
+ Splash Screen Cyberpunk + WebSockets dinámicos + Mercado Pago Perú.

Enfoque B2C hogareño con jerarquía parental estricta:
  · ADMIN_CASA  → Padre/Madre. Control absoluto.
  · GESTOR_CASA → Tutor. Solo supervisión, sin comandos de energía.
  · USUARIO     → Hijo/Hija. Solo lectura de sus propios equipos.
...
En Render.com:
    Start Command → python Servidor_Universal_Aegis_Family_OS.py
    (o gunicorn -k uvicorn.workers.UvicornWorker Servidor_Universal_Aegis_Family_OS:app)
"""
...
# Interruptor de negocio: TRUE → modo SaaS con prueba de 30 días.
...
# Bypass perpetuo VIP para el búnker familiar (sin conteo de días, sin paywall).
CODIGOS_VIP = {"FAM123"}
    ...
    def __init__(self):
        ...
        if self.es_postgres:
            try:
                ...
                raise RuntimeError(
                    "DATABASE_URL configurada pero 'psycopg2' no instalado."
                ) from e
...
def init_db():
    ...
    if conexion.uno()["total"] == 0:
        ...
        if PIN_SEMILLA_MASTER == "9999":
            log.warning("⚠ Master PIN por defecto '9999'. Configura AEGIS_MASTER_PIN.")

    # Casa semilla + búnker VIP familiar
    conexion.ejecutar("SELECT COUNT(*) AS total FROM casas")
    if conexion.uno()["total"] == 0:
        for nombre, codigo, admin, pin_admin, avatar in [
            ("Búnker Familiar", "FAM123", "Manuel", "1234", "👨‍💻"),
        ]:
            conexion.ejecutar(
                "INSERT INTO casas (nombre, codigo_invitacion, creado_en) VALUES (?, ?, ?)",
                (nombre, codigo, datetime.utcnow().isoformat()),
                es_insert=True,
            )
            ...
            conexion.ejecutar(
                "INSERT INTO usuarios (casa_id, nombre, pin_hash, salt, rol, avatar) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (casa_id, admin, hash_pin(pin_admin, salt), salt, ROL_ADMIN_CASA, avatar),
            )
            ...
            log.info("Búnker familiar creado. Código: %s", codigo)
...
# ===========================================================================
# 7 · SPLASH SCREEN + DASHBOARD HTML EMBEBIDO
# ===========================================================================
...
<style>
  *{-webkit-tap-highlight-color:transparent;box-sizing:border-box}
  ...
  .avatar-btn{transition:all .18s ease;cursor:pointer;border-radius:14px}
  .avatar-btn:hover{transform:scale(1.2);border-color:#22d3ee;background-color:rgba(34,211,238,.2)}
  #mapa{width:100%;height:100%;border-radius:20px;filter:hue-rotate(180deg) saturate(1.2) brightness(.8)}
  .leaflet-container{background:#020617}
</style>
</head>
<body>

<!-- ================= SPLASH SCREEN TÁCTICO ================= -->
<div id="splashScreen"
  ...
  <div class="relative fade-up flex flex-col items-center gap-10 px-6 text-center">
    ...
    <div>
      ...
      <p class="mt-4 text-[10px] md:text-xs tracking-[0.45em] text-cyan-400/70">
        NÚCLEO OPERATIVO · v12.0 · FAMILIA SEGURA
      </p>
    </div>
  ...
  <div class="max-w-[1700px] mx-auto flex flex-col gap-6">
    ...
    <header class="glass-card p-6 flex flex-wrap justify-between items-center gap-4">
      ...
      <div class="flex items-center gap-3">
        <span id="casaBadge" class="font-mono text-[10px] tracking-[0.3em] text-cyan-400/70">CASA: ---</span>
    ...
    <div id="faseAcceso" class="glass-card p-8 md:p-12 flex flex-col gap-6 items-center text-center">
      ...
      <div class="w-full max-w-md flex flex-col gap-3 mt-4">
        <input id="inputCodigoCasa" type="text" maxlength="6" placeholder="FAM123"
    ...
    <div id="faseDashboard" class="hidden grid grid-cols-1 xl:grid-cols-12 gap-6">
      <div class="xl:col-span-4 glass-card p-6 flex flex-col gap-4">
        ...
        <div id="listaDispositivos" class="space-y-3 max-h-[75vh] overflow-y-auto pr-1"></div>
      </div>
      ...
      <div class="xl:col-span-8 flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="glass-card p-6 flex flex-col gap-3">
            ...
            <div class="w-full h-[260px] rounded-2xl overflow-hidden border border-slate-800">
              <div id="mapa"></div>
            </div>
          </div>
...
/* =====================================================================
   SPLASH → DASHBOARD (transición fluida)
   ===================================================================== */
function conversar() {
...
/* =====================================================================
   WEBSOCKET DINÁMICO (Render-safe: usa location.host)
   ===================================================================== */
function initSocket() {
  try {
    ...
    socketWS.onopen = () => console.log("[AEGIS] WS conectado");
    ...
    socketWS.onclose = () => {
      console.log("[AEGIS] WS cerrado, reintentando en 5s…");
...
function actualizarMapa(lat, lng, nombre) {
  ...
  if (markerDispositivo) {
    markerDispositivo.setLatLng(pos);
  } else {
    markerDispositivo = L.marker(pos).addTo(mapaLeaflet);
  }
  markerDispositivo.bindPopup("<b>" + nombre + "</b>");
  mapaLeaflet.setView(pos, 14);
}
...
async function crearCasa() {
  ...
  if (!nombre_casa || !nombre_admin || pin_admin.length < 4) {
    alert("Completa todos los campos (PIN mín. 4 dígitos)");
    return;
  }
  try {
...
function entrarAlDashboard() {
  ...
  if (usuarioActual.rol === "ADMIN_CASA") btnVinc.classList.remove("hidden");

  document.getElementById("avisoGestor").classList.toggle(
    "hidden", usuarioActual.rol !== "GESTOR_CASA"
  );
  document.getElementById("panelComandos").style.opacity =
    (usuarioActual.rol === "ADMIN_CASA") ? "1" : "0.35";
  document.getElementById("panelComandos").style.pointerEvents =
    (usuarioActual.rol === "ADMIN_CASA") ? "auto" : "none";

  mostrar("faseDashboard");
  cargarDispositivos();
  if (timerTelemetria) clearInterval(timerTelemetria);
  timerTelemetria = setInterval(cargarDispositivos, 12000);
}
...
async function cargarDispositivos() {
  try {
    ...
    pintarDispositivos();
  } catch (e) { /* paywall ya manejado */ }
}

const ICONOS = {
  pc: "💻", smartphone: "📱", tablet: "📟", smart_tv: "📺", enchufe: "🔌"
};
...
function pintarDispositivos() {
  ...
  dispositivos.forEach(d => {
    ...
    el.className = "w-full text-left hud-box p-4 transition " +
      (activo ? "border-cyan-400" : "hover:border-cyan-500/60");
    el.innerHTML =
      '<div class="flex items-center gap-3">' +
...
/* =====================================================================
   COMANDOS DE ENERGÍA (bloqueo parental vía HTTP 403)
   ===================================================================== */
async function ejecutarComando(accion) {
  ...
  if (usuarioActual.rol !== "ADMIN_CASA") {
    alert("Tu rol no permite enviar comandos de energía (HTTP 403)");
    return;
  }
  try {
    ...
    if (res.status === 403) {
      ...
      alert("⛔ " + (d.detail || "Permiso denegado"));
      return;
    }
...
async function generarCodigoVinculacion() {
  try {
    ...
    document.getElementById("urlVincular").innerText =
      location.origin + "/vincular/" + data.codigo;
...
Se ejecuta en cada dispositivo remoto. Reporta telemetría y ejecuta comandos.
...
def loop():
    if not AEGIS_TOKEN:
        _log("ERROR: sin token")
        sys.exit(1)
    _log("Agente arrancado → " + AEGIS_URL)
    while True:
        try:
            requests.post(AEGIS_URL + "/agente/telemetria",
                          json=telemetria(), headers=HEADERS, timeout=10)
        except Exception as e:
            _log("Telemetría falló: " + str(e))
        try:
            r = requests.get(AEGIS_URL + "/agente/comando",
                             headers=HEADERS, timeout=10)
            if r.status_code == 200:
                d = r.json()
                if d.get("comando_id") and d.get("accion"):
                    ejecutar(d["accion"])
                    requests.post(AEGIS_URL + "/agente/comando/" +
                                  str(d["comando_id"]) + "/completado",
                                  headers=HEADERS, timeout=10)
        except Exception as e:
            _log("Comando falló: " + str(e))
...
async def lifespan(_app: FastAPI):
    log.info("AEGIS FAMILY OS v12.0 · MODO_COMERCIAL=%s", MODO_COMERCIAL)
    init_db()
...
app = FastAPI(
    title="Aegis Family OS — Servidor Universal",
    version="12.0",
    lifespan=lifespan,
)
...
async def _verificar_suscripcion(casa: Optional[dict], usuario: dict):
    """Candado 402 + Bypass VIP para códigos familiares privilegiados."""
    ...
    # REGLA VIP FAMILIAR: código 'FAM123' → bypass perpetuo
    if (casa.get("codigo_invitacion") or "").upper() in CODIGOS_VIP:
        return
...
def telemetria_del_servidor_local() -> dict:
    ...
    try:
        ...
        ram_val = "N/D"
    try:
        disco_val = f"{psutil.disk_usage(os.path.abspath(os.sep)).percent}%"
    except Exception:
        disco_val = "N/D"
    try:
...
async def generar_codigo(
    ...
    with db() as conexion:
        ...
        conexion.ejecutar(
            "INSERT INTO codigos_vinculacion "
            "(casa_id, codigo, nombre_sugerido, tipo_sugerido, expira_en, usado) "
            "VALUES (?, ?, ?, 'pc', ?, 0)",
...
async def controlar_energia(
    ...
    if usuario["rol"] == ROL_USUARIO:
        raise HTTPException(
            status_code=403,
            detail="Tu rol (Hijo/Hija) NO puede ejecutar comandos de energía sobre el sistema",
        )
...
# ===========================================================================
# 18 · VINCULACIÓN AUTÓNOMA (multidispositivo extendido)
# ===========================================================================
...
h1{{font-family:'Orbitron',sans-serif;font-size:26px;color:#67e8f9;letter-spacing:.12em;
margin:0 0 8px;text-shadow:0 0 24px rgba(34,211,238,.8)}}
...
text-transform:uppercase;text-decoration:none;color:#a5f3fc;text-align:center;
justify-content:center;border:1px solid rgba(34,211,238,.6);
...
<li><b>Smart TV / Enchufe:</b> usar el agente o el webhook compatible.</li>
</ul>
...
def vincular_autonomo(codigo: str, request: Request):
    ...
    if not codigo or len(codigo) != 6 or not codigo.isdigit():
        return HTMLResponse(_pagina_error("Código inválido", "El código debe tener 6 dígitos."),
                            status_code=400)
    ...
    with db() as conexion:
        ...
        if not fila:
            return HTMLResponse(_pagina_error("Código no encontrado",
                                "Verifica el código que te compartió el Administrador."),
                                status_code=404)
        if fila["usado"]:
            return HTMLResponse(_pagina_error("Código ya utilizado",
                                "Pide uno nuevo desde el panel."), status_code=400)
        try:
            expira = datetime.fromisoformat(fila["expira_en"])
        except (ValueError, TypeError):
            expira = datetime.utcnow() - timedelta(seconds=1)
        if expira < datetime.utcnow():
            return HTMLResponse(_pagina_error("Código expirado",
                                "Caducó a los 10 minutos. Genera uno nuevo."), status_code=400)

        nombre_auto = (fila.get("nombre_sugerido") or "").strip() or \
                      f"Equipo-{secrets.token_hex(2).upper()}"
    ...
    log.info("Vinculación autónoma: id=%s, ip=%s", dispositivo_id, ip_visitante)
    return HTMLResponse(_pagina_vincular(nombre_auto, token, codigo, script))


# ===========================================================================
# 19 · MERCADO PAGO PERÚ — WEBHOOK (FIX: match contra codigo_invitacion)
# ===========================================================================
...
async def webhook_mercado_pago(request: Request):
    ...
    # external_reference AHORA ES EL CÓDIGO ALFANUMÉRICO DE 6 CARACTERES
    external_ref = (
        payload.get("external_reference")
        or data.get("external_reference")
        or ""
    )
    external_ref = str(external_ref).strip().upper()

    casa: Optional[dict] = None
    if external_ref:
        casa = buscar_casa_por_codigo(external_ref)

    # Fallback: consultar la API de MP si solo llegó payment_id
    ...
    log.info("Suscripción renovada. Casa=%s (%s) · Pago=%s",
             casa["nombre"], casa["codigo_invitacion"], payment_id)
...
def agente_obtener_comando(x_device_token: str = Header(None)):
    ...
    with db() as conexion:
        ...
        if comando:
            conexion.ejecutar("UPDATE comandos SET estado = 'ENVIADO' WHERE id = ?",
                              (comando["id"],))
            conexion.commit()
    if not comando:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await websocket.send_json({"event": "BIENVENIDA", "version": "12.0"})
        while True:
            try:
...
# ===========================================================================
# 22 · ENTRYPOINT (INDENTACIÓN AL RAS · RENDER-SAFE)
# ===========================================================================
if __name__ == "__main__":
    import uvicorn
